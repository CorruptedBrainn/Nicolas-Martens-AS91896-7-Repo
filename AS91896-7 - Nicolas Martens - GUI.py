    ...
    match mode:
        case "d":
            ...
            if confirm == False:
                ...
                if str(int(rowNum)) in firstRow and rowNum.is_integer():
                    ...
                    rowFile = open(fr"{genDir}\AS91897ROW.txt","w")
        ...
        case "w":
            ...
            if confirm == False:
                ...
                dataString = ""
                dataFile = open(fr"{genDir}\AS91897DATA.txt","r")
...
def main():
    ...
    if os.path.exists(fr"{genDir}\AS91897ROW.txt") == False:
        # This checks if the AS91897ROW file exists
        # The file is used to store all the valid rows - row numbers will be deleted from here

        rowFile = open(fr"{genDir}\AS91897ROW.txt", "a")
        rowFile.write("1")
        rowFile.close()
        # Write "1" as an initial value - this shows the next free spot to edit in

    if os.path.exists(fr"{genDir}\AS91897DATA.txt") == False:
        # This checks if the AS91897DATA file exists
        # The file is used to store every single entry - values here are never deleted, only added and edited

        dataFile = open(fr"{genDir}\AS91897DATA.txt", "a")
    ...
    while True:
        ...
        rowFile = open(fr"{genDir}\AS91897ROW.txt","r")
